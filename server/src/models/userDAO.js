        ...
        database.getConnection((error, connection) => {
          ...
          connection.query(sql, [username], (error, results) => {
            ...
            if (error) {
              ...
              return {
                username: results[0].NomUtilisateur,
                password: results[0].MotDePasse,
              };
            } else {
              resolve(null);
            }
...
async function createUser(NomUtilisateur, MotDePasse) {
  return new Promise((resolve, reject) => {
    let sql =
      'INSERT INTO Utilisateur (NomUtilisateur, MotDePasse) VALUES (?, ?);';
    database.getConnection((error, connection) => {
      if (error) {
...
async function updateUser(nom, MotDePasse) {
  ...
  database.getConnection((error, connection) => {
    if (error)
      console.error('Database connection error on updateUser', error.message);
